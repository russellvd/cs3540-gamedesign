...
namespace TheVisualEngineLite
{
    public class TVELiteHub : EditorWindow
    {
        ...
        string autorunPath;
        string assetFolder = "Assets/BOXOPHOBIC/The Visual Engine Lite";
        ...
        [MenuItem("Window/BOXOPHOBIC/The Visual Engine Lite/Hub", false, 1010)]
        public static void ShowWindow()
        {
            window = GetWindow<TVELiteHub>(false, "The Visual Engine Lite", true);
        ...
        void OnEnable()
        {
            ...
            searchFolders = AssetDatabase.FindAssets("The Visual Engine Lite");

            for (int i = 0; i < searchFolders.Length; i++)
            {
                if (AssetDatabase.GUIDToAssetPath(searchFolders[i]).EndsWith("The Visual Engine Lite.pdf"))
                {
                    assetFolder = AssetDatabase.GUIDToAssetPath(searchFolders[i]);
                    assetFolder = assetFolder.Replace("/The Visual Engine Lite.pdf", "");
                }
            }
            ...
            for (int i = 0; i < searchFolders.Length; i++)
            {
                if (AssetDatabase.GUIDToAssetPath(searchFolders[i]).EndsWith("User.pdf"))
                {
                    ...
                    userFolder += "/The Visual Engine Lite";
                }
            }
            ...
            bannerText = "The Visual Engine Lite";
        ...
        void OnGUI()
        {
            ...
            var unityVersionSupported = TVELiteUtils.IsVersionSupported();

            if (unityVersionSupported != "")
            {
                EditorGUILayout.HelpBox(unityVersionSupported, MessageType.Error, true);
            ...
            {
                EditorGUILayout.HelpBox("Welcome to the The Visual Engine Lite asset! Choose the render pipeline used in your project to make the shaders compatible with the current pipeline!", MessageType.Info, true);
            ...
            {
                EditorGUILayout.HelpBox("The Visual Engine Lite is installed for the " + pipelineCurrent + "! Use the additional setting below to restart the render pipeline setup.", MessageType.Info, true);
        ...
        void SetDefineSymbols()
        {
            ...
            if (!defineSymbols.Contains("THE_VISUAL_ENGINE_LITE"))
            {
                defineSymbols += ";THE_VISUAL_ENGINE_LITE;";
